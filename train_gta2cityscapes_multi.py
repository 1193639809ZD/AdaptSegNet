...
def get_arguments():
    ...
    parser.add_argument("--lambda-seg", type=float, default=0.0, help="lambda_seg.")
    parser.add_argument(
        "--lambda-adv-target1",
        type=float,
        default=0.0,
    ...
    parser.add_argument(
        "--not-restore-last", action="store_true", help="Whether to not restore last (FC) layers."
    )
    parser.add_argument(
        "--num-classes",
        type=int,
    ...
    parser.add_argument(
        "--save-num-images", type=int, default=SAVE_NUM_IMAGES, help="How many images to save."
    )
    parser.add_argument(
        "--save-pred-every",
        type=int,
    ...
    # parser.add_argument(
    #     "--snapshot-dir", type=str, default=SNAPSHOT_DIR, help="Where to save snapshots of the model."
    # )
    parser.add_argument(
        "--snapshot-dir",
        type=str,
        default="./snapshots/GTA2Cityscapes_single_lsgan",
    ...
    for i_iter in range(args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            loss_adv_target1 = bce_loss(
                D_out1, Variable(torch.FloatTensor(D_out1.data.size()).fill_(source_label)).cuda(args.gpu)
            )

            loss_adv_target2 = bce_loss(
                D_out2, Variable(torch.FloatTensor(D_out2.data.size()).fill_(source_label)).cuda(args.gpu)
            )
            ...
            loss_D1 = bce_loss(
                D_out1, Variable(torch.FloatTensor(D_out1.data.size()).fill_(target_label)).cuda(args.gpu)
            )

            loss_D2 = bce_loss(
                D_out2, Variable(torch.FloatTensor(D_out2.data.size()).fill_(target_label)).cuda(args.gpu)
            )
        ...
        if i_iter >= args.num_steps_stop - 1:
            print("save model ...")
            torch.save(
                model.state_dict(), osp.join(args.snapshot_dir, "GTA5_" + str(args.num_steps_stop) + ".pth")
            )
            torch.save(
                model_D1.state_dict(),
